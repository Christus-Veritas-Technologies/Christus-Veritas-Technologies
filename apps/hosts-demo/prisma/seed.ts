...
// Prisma 7 "client" engine requires a database adapter — same pattern as prisma.ts
...
async function main() {
  ...
  // This means re-deploying with SEED_DATABASE_URL set won't wipe existing bookings.
  const existingRooms = await prisma.room.count();
  if (existingRooms > 0) {
    console.log(`✓ Database already seeded (${existingRooms} rooms found) — skipping.`);
    return;
  }
