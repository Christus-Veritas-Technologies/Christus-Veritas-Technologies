...
export default function DashboardPage() {
  ...
  if (loading) {
    return <div className="p-8 text-[var(--text-secondary)]">Loading overview...</div>;
  }
    ...
    <div className="p-6 max-w-6xl mx-auto">
      <div className="flex justify-between items-center mb-6">
        <div>
          ...
          <p className="text-sm text-[var(--text-secondary)]">{format(new Date(), "EEEE, d MMMM yyyy")}</p>
        </div>
        <Button asChild>
          ...
          <CardContent>
            <ResponsiveContainer width="100%" height={220}>
              <BarChart data={stats.revenueChart} margin={{ left: -10 }}>
                ...
                />
                <Tooltip
                  formatter={(v: number) => [`R${v.toLocaleString("en-ZA")}`, "Revenue"]}
                  contentStyle={{ fontSize: 12, border: "1px solid var(--border)", borderRadius: 6, background: "var(--bg-surface)" }}
                />
          ...
          <CardContent className="space-y-2">
            {[
              ...
              <Link
                key={label}
                href={href}
                className="flex items-center justify-between p-3 rounded-lg border border-[var(--border)] hover:border-[var(--accent)]/50 hover:bg-[var(--bg-subtle)] transition-colors text-sm font-medium text-[var(--text-primary)]"
              >
                {label}
              ...
              <thead>
                <tr className="border-b border-[var(--border)]">
                  {["Reference", "Guest", "Room", "Dates", "Amount", "Status"].map((h) => (
                    <th key={h} className="py-2 px-3 text-left text-xs font-semibold text-[var(--text-secondary)] uppercase tracking-wide">{h}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
